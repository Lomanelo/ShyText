...
Motion: interruptible spring press (0.96), hold fill tracks the finger (~720ms) then a short squash, layout spring when the live dock / you’re-here card / chat bubbles move. Live dots beat. OTP slots pop on fill. Story bars fill left-to-right. Lists skip enter. Honor Reduce Motion (hold becomes tap).
...
People: only after you check in here; mood chips update live; Other opens a 40-character status; swipe left to send (button remains); Send a ShyText is a bottom sheet, never a silent skip
...
Settings: grouped inset rows with chevron
...
Chat: iMessage-style bubbles; pill composer with circular send
